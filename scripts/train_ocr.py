...
OCR 识别训练入口：
1. 使用裁剪后的车牌图训练 CRNN + CTC 模型。
2. 自动修正标签文件中的本地旧路径，适配云端训练。
3. 输出最佳权重和验证指标。
...
def resolve_label_file(label_file: Path, split_name: str, image_root: Path, output_file: Path) -> Path:
    ...
    if missing_images:
        ...
        raise FileNotFoundError(
            f"{label_file} 中有图片在云端找不到，前 10 个示例：\n{preview}"
        )
...
def main() -> None:
    # ==================== 固定参数区 ====================
    charset_path = PROJECT_ROOT / "ocr" / "charset.txt"
    train_label = PROJECT_ROOT / "ocr" / "train_label.txt"
    val_label = PROJECT_ROOT / "ocr" / "val_label.txt"
    image_root = PROJECT_ROOT / "ocr" / "images"
    output_dir = PROJECT_ROOT / "runs_ocr"
